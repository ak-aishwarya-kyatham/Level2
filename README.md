...
### Exact Test Commands
To run the standard offline test suite (excluding live ## 🧪 Testing & Quality Assurance
...
```bash
# Standard Offline Suite (Fast, isolated with tmp_path and mocks)
...
# Category 5: Optional Live Ollama Pytest Suite
...
Follow these step-by-step commands to reproduce all test suite runs, execution traces, and evaluation metrics:

### 1. Run the Offline Test Suite
Run all unit and deterministic integration tests completely offline without requiring running daemons (Ollama, MongoDB, Redis, or Qdrant):
```bash
cd backend
python -m pytest -q -p no:cacheprovider
```
* **What it proves**: 100% pass rate across state graph orchestration, Policy action parsing, Reflection verifications, and memory fallback handlers in under 15 seconds.

### 2. Generate and Verify a Live Agent Trace
Generate an authentic, unmocked multi-agent execution trace:
```bash
# Step A: Ensure Ollama is running locally with qwen2.5:3b
ollama run qwen2.5:3b

# Step B: Execute the standalone live trace generator
cd backend
python scripts/run_live_trace.py

# Step C: Verify the trace file before trusting it as live evidence
python scripts/verify_live_trace.py
```
* **Trace Artifact Output**: [`tests/live_agent_trace.txt`](file:///c:/Users/AishwaryaK/Desktop/agent/backend/tests/live_agent_trace.txt)
* **What it demonstrates**: Demonstrates live LLM decision-making (`qwen2.5:3b`), MCP tool execution (`search_live_news` / `compare_news_sources`), grounded summary synthesis, and Reflection Agent validation when Ollama is active locally.
* **Reviewer Verification**: Reviewers should run `python scripts/verify_live_trace.py` against the committed trace file to confirm that LLM fallback was inactive and genuine live model inference occurred.

### 3. Run a Real Live Evaluation Benchmark
Execute a live evaluation run across labeled ground-truth queries (`backend/app/data/eval_dataset.json`) to measure wall-clock latency and un-floored routing accuracy:
```bash
...
* **What it demonstrates**: Calculates real wall-clock latency using `time.perf_counter()`, un-floored routing accuracy against `eval_dataset.json`, token-overlap faithfulness, and MRR@10.

---

### Deterministic Trace vs. Live Trace Artifacts
- **`tests/simulated_integration_trace.txt`**: Generated by `DeterministicWorkflowSimulator` in `test_full_agent_workflow.py`. All LLM policy selection, reflection, MCP tools, and synthesis are mocked. **Proves state-graph wiring, state transitions, and edge routing logic only.**
- **`tests/live_agent_trace.txt`**: Produced by `scripts/run_live_trace.py` against a live Ollama model (`qwen2.5:3b`) and a real stdio MCP server session. **Proves live LLM tool selection and reflection-based self-critique; the committed trace demonstrates the full detect-revise-reassess loop including cases where correction is not achieved within the iteration cap.**
  * *Note*: Ollama must be running locally with `qwen2.5:3b` pulled for this trace to reflect live behavior; the committed file reflects one specific run at a specific time and is not regenerated automatically.
