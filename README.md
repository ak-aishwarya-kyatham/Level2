...
All offline tests execute in under 30 seconds with zero external network or database requirements.

```powershell
# 1. Run standard offline test suite (excluding live external service tests)
python -m pytest -q -p no:cacheprovider

# 2. Run real MCP stdio ClientSession integration test
python -m pytest tests/test_mcp_real_session.py -v

# 3. Verify live agent trace
python scripts/verify_live_trace.py

# 4. Run live trace script (Requires Ollama running on localhost:11434)
python scripts/run_live_trace.py
```

### Trace Artifacts
- **`tests/simulated_integration_trace.txt`**: Deterministic simulation verifying state graph wiring, state transitions, and tool loop-backs.
- **`tests/live_agent_trace.txt`**: Live unmocked trace proving real stdio MCP transport, dynamic tool discovery, LLM policy action selection, and factual reflection.
