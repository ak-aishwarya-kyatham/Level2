...
* **What it demonstrates**: Proves authentic live LLM decision-making (`qwen2.5:3b`), stdio JSON-RPC tool invocation (`search_live_news`), grounded summary synthesis, and Reflection Agent validation.
...
- **`tests/live_agent_trace.txt`**: Produced by `scripts/run_live_trace.py` against a live Ollama model (`qwen2.5:3b`) and a real stdio MCP server session. **Proves authentic live LLM reasoning, stdio JSON-RPC tool calls, and grounded response synthesis.**
