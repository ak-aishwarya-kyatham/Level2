...
### Trace Artifacts (Locally Generated — Not Committed)

Live trace files must be generated locally and are excluded from the repository via `.gitignore`.
This prevents stale, pre-refactor traces from misrepresenting the current MCP architecture.

- **`tests/live_agent_trace.txt`** *(local only)*: Generated by `run_live_trace.py` — proves real stdio MCP transport, dynamic tool discovery, LLM policy action, and grounded reflection.
- To regenerate: `python scripts/run_live_trace.py` (requires Ollama + active MCP session)
