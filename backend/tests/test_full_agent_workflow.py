...
async def test_full_agent_workflow_end_to_end():
    ...
    import os
    trace_file = os.path.join(os.path.dirname(__file__), "simulated_integration_trace.txt")
    with open(trace_file, "w", encoding="utf-8") as f:
        f.write("# NOTE: Generated by DeterministicWorkflowSimulator in test_full_agent_workflow.py.\n")
        f.write("# Policy selection, reflection, MCP tool execution, and synthesis are all mocked.\n")
        f.write("# This proves state-graph wiring only, not live model behavior.\n")
        f.write("# See live_agent_trace.txt for a real unmocked run.\n\n")
        f.write("========== AGENT TRACE ==========\n\n")
    ...
    print("\n" + "=" * 70)
    print("SIMULATED INTEGRATION WORKFLOW EXECUTION TRACE")
    print("=" * 70)
    for step in simulator.trace:
        print(step)
    print("=" * 70 + "\n")
